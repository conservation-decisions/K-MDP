...
m <- fisheries_matrices(states, actions, observed_states, reward_fn, f, sigma_g, sigma_m)
  ...
  for (f in 1:n_a) {
    ...
    new_P_file = paste("P",f,n_s,n_a,".txt",sep="_");
    write.table(transition[,,f], file=new_P_file, row.names=FALSE, col.names=FALSE)
    
  }
  
  
  R_file = paste("R",n_s,n_a,".txt",sep="_");
  write.table(reward, file=R_file, row.names=FALSE, col.names=FALSE)
